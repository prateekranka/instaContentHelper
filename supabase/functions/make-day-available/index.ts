...
type MakeDayAvailableRequest = {
  creator_id?: string;
  scheduled_date?: string;
  daily_card_id?: string;
};

type MakeDayAvailableAdminClient = SupabaseAdminClient & {
  rpc: (
    functionName: string,
...
export async function handleMakeDayAvailableRequest(
  request: Request,
  ...
  const admin =
    (dependencies.createAdminClient
      ...
      })) as MakeDayAvailableAdminClient;

  // Creator may run Available on Today; owner/editor retain access for Admin path.
  ...
  let body: MakeDayAvailableRequest;
  try {
  ...
  if (!creatorID || !UUID_PATTERN.test(creatorID)) {
    return jsonResponse({ error: "invalid_make_day_available_payload" }, 400);
  }
  ...
  const { data, error } = await admin.rpc("make_day_available", { payload });
  if (error) {
    return jsonResponse({ error: "make_day_available_failed" }, 500);
  }
  ...
  return jsonResponse({
    ...
    week_is_soft_locked: result.week_is_soft_locked === true,
  });
}
...
function stableError(code: string): string {
  switch (code) {
    case "invalid_make_day_available_payload":
    case "daily_card_not_found":
    case "daily_card_not_draft":
    case "daily_card_incomplete":
      return code;
    default:
      return "make_day_available_failed";
  }
}

if (import.meta.main) {
  Deno.serve((request) => handleMakeDayAvailableRequest(request));
}
