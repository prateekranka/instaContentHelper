...
Deno.test("storyboardRowsForCard falls back to top-level GeneratedDailyCard timelines", () => {
  const rows = storyboardRowsForCard({
    title: "Strength story",
    content_pillar: "gym",
    scene_list: [{ title: "Opening", duration: "0-3 sec" }],
    shot_timeline: [{
      timestamp: "0-3 sec",
      shot: "Top-level talking head",
      video_portion: "Direct eye contact from top-level.",
    }],
    voiceover_timeline: [{ voiceover: "Top-level voiceover." }],
    on_screen_text_timeline: [{ on_screen_text: "TOP LEVEL TEXT" }],
  });

  assertEquals(rows.length, 1);
  assertEquals(rows[0].visual_shot, "Top-level talking head");
  assertEquals(rows[0].what_to_show, "Direct eye contact from top-level.");
  assertEquals(rows[0].audio_dialogue, "Top-level voiceover.");
  assertEquals(rows[0].on_screen_text, "TOP LEVEL TEXT");
});
