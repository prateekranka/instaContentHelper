# SwiftUI Design System And Implementation Spec: Mamta Content OS V2 Full MVP

Generated with `build-ios-apps:swiftui-ui-patterns`.

Inputs:

- `docs/layers-conceptual-model-mamta-content-os-v2.md`
- `docs/layers-interaction-flow-mamta-content-os-v2.md`
- `docs/layers-surface-mamta-content-os-v2-full-mvp.md`
...
- Earlier Mamta Daily Mode board: `/Users/prateekranka/.codex/generated_images/019e91d6-f797-7072-bc8c-b5db7482e6cf/ig_0b70b2531334d988016a21af17bf7881919860e24aa72a9200.png`
- Earlier Prateek Weekly Control board: `/Users/prateekranka/.codex/generated_images/019e91d6-f797-7072-bc8c-b5db7482e6cf/ig_0b70b2531334d988016a21af9a792881918ddeece7f9c76aa5.png`
- Earlier Intelligence System board: `/Users/prateekranka/.codex/generated_images/019e91d6-f797-7072-bc8c-b5db7482e6cf/ig_0b70b2531334d988016a21b02438f88191b438814a07d5f43c.png`
...
Mamta Content OS is a native iPhone app that turns Prateek's weekly setup and curated source intelligence into one prepared, shootable Daily Card for Mamta.

The app has two visible personalities:

- Mamta Mode: a quiet daily decision product. It should feel like opening a premium fitness journal and seeing exactly what to shoot today.
- Prateek/Admin Mode: an editorial operating surface. It should help Prateek prepare, review, publish, and improve the week without feeling like SaaS analytics.
...
- `Approve`: an extracted Pattern, Trend, Audio Option, or Idea fits Mamta.
- `Publish`: Mamta can now see the Weekly Plan or update.
- `Daily Card`: the scheduled daily package.
- `Today`: the Mamta-facing placement of the current Daily Card.
- `Weekly Setup`: Prateek's weekly input packet.
- `Weekly Plan`: the 7-day plan generated from setup and reviewed by Prateek.
...
The revised visual direction is `Training Folio`: a private editorial folio for Mamta's week, not a data console.

Every screen should operate in one density mode:

- `Glance`: one decision or one next action. Use for Mamta Today, Publish Confirmation, and Archive summaries.
...
- Mamta screens show one hero idea, maximum two metadata chips, maximum two visible actions, and one source sentence at most.
...
- Mamta decision blocks: minimum 56 point tap height.
...
- Mamta: quiet avatar/monogram, title, date/context line, small settings control.
...
Use on Weekly Plan rows and admin Daily Card Review. On Mamta Today, collapse this to a small sentence such as `Inspired by a fitness-after-60 pattern`.
...
Use lightly on admin/intelligence screens. Do not show fit score prominently on Mamta Today.
...
- Mamta's device should default to Mamta Mode.
- Prateek's device should default to Admin Mode.
- A small Settings/Control icon can expose mode switching for Prateek.
- Mamta can edit if needed, but advanced controls are tucked behind settings and deliberate confirmation.
...
`MamtaSheetDestination`:

```swift
enum MamtaSheetDestination: Identifiable, Hashable {
...
- Show one current Published Daily Card and let Mamta decide what happens today.
...
- No fit score as a primary Mamta element.
...
- Let Mamta make a decision quickly, including lower-effort backups.
...
- Do not ask Mamta to explain before presenting backups.
...
- Original card is never mutated until Mamta/admin chooses `Use this`.
...
Primary user: Prateek. Secondary: Mamta view-only.
...
1. `MamtaPreviewCard`: exactly what Mamta will see.
...
- The Mamta preview comes before admin controls.
...
- Change only affected future cards without destabilizing Mamta's week.
...
- If Mamta currently has the card cached, update non-disruptively and show a revision marker next open.

### 8. References

Primary user: Prateek/scout.
...
- Do not foreground external creators over Mamta adaptation.
- Trend freshness is visible but secondary to Mamta fit.
...
- Pattern: title, type, summary, Mamta adaptation, complexity, fit, source references, used count.
- Trend: summary, first/last seen, timing recommendation, region/niche, hook/visual/caption pattern, Mamta adaptation, saturation note, fit, audio options.
...
Primary user: Prateek. Mamta advanced access.
...
Primary users: Mamta and Prateek.
...
- Mamta sees personal history; Prateek sees history plus learning actions.
...
MamtaContentOS/
  App/
    MamtaContentOSApp.swift
    MamtaContentOSAppView.swift
    AppState.swift
    AppMode.swift
    AppServices.swift
...
Tables should include `workspace_id` and `creator_id` even though the V1 UI only exposes Mamta.
...
- Failed completion sync should create an admin-visible sync warning, not a scary Mamta error.
...
Use local notifications on Mamta's phone.

Flow:

1. Prateek publishes a Weekly Plan.
2. Mamta's phone syncs the published plan.
3. Mamta's phone schedules gentle daily local notifications.
...
2. Mamta Today, Package Detail, Decision Sheet, Archive with local fixtures.
...
10. Notification scheduling on Mamta's device.
11. Learning Summary and Archive feedback loop.
12. End-to-end TestFlight proof: one Reference -> approved Pattern/Trend/Audio -> Weekly Plan -> Daily Card -> Mamta decision -> Archive/Learning.

## Acceptance Checks For First SwiftUI Pass

- Mamta launches into Today without seeing admin machinery.
- Prateek launches into Weekly/Admin mode without losing access to Today Preview.
- Today can be used offline from cache.
- A completed day means a decision was made, not necessarily that Mamta posted.
