# Runtime Config and Device Pairing: Creator Content OS V2
...
- `CreatorContentOS/Config/Runtime.xcconfig`
- optional local override `CreatorContentOS/Config/LocalRuntime.xcconfig`
- example file `CreatorContentOS/Config/LocalRuntime.sample.xcconfig`
...
- service: `com.creatorcontenthelper.runtime`
...
`CreatorContentOSApp` creates an `AppRuntime` on launch:
...
The current schema RLS helpers are Supabase Auth based. Device-token authorization should be completed before relying on direct live reads/writes from Creator's phone.
