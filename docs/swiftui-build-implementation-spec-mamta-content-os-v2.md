# SwiftUI Build Implementation Spec: Mamta Content OS V2
...
- `docs/canonical-training-folio-boards-mamta-content-os-v2.md`
...
- Training Folio Mamta Daily Mode: `/Users/prateekranka/.codex/generated_images/019e91d6-f797-7072-bc8c-b5db7482e6cf/ig_0b70b2531334d988016a21baf5b41481919325834854242dec.png`
- Training Folio Prateek Weekly Control: `/Users/prateekranka/.codex/generated_images/019e91d6-f797-7072-bc8c-b5db7482e6cf/ig_0b70b2531334d988016a21bbdccef48191b828f54a07d944bd.png`
- Training Folio Intelligence System: `/Users/prateekranka/.codex/generated_images/019e91d6-f797-7072-bc8c-b5db7482e6cf/ig_0b70b2531334d988016a21be44255c81918f32f4e8dd2ab875.png`

Superseded boards:

- Earlier Mamta/Weekly/Intelligence boards remain in the generated image folder as history only. Do not implement their denser row layouts, package previews on Today, count-first Intelligence rows, or data-heavy admin treatments.
...
`Reference -> approved Pattern/Trend/Audio Option -> Weekly Plan -> Published Daily Card -> Mamta Today decision -> Archive entry`
...
final class AppState {
  ...
  init(
    ...
    currentCreatorID: Creator.ID = .fixtureMamta,
    currentMember: Member = .fixtureMamta
  ) {
...
- Mode switching is allowed, but Mamta-facing UI defaults to Mamta Mode.
...
Create `App/MamtaContentOSAppView.swift`.
...
Create `Navigation/MamtaNavigation.swift`.

```swift
import SwiftUI

enum MamtaTab: String, CaseIterable, Identifiable, Hashable {
  case today
  case week
  case archive
  case settings
...
`MamtaShellView` contract:

- `TabView(selection:)`.
- `NavigationStack(path:)` per tab.
- `navigationDestination(for: MamtaRoute.self)`.
...
- Mamta completion writes local state immediately.
- If offline, queue the mutation.
- A sync failure is admin-visible, not a Mamta-blocking error.
...
- Presents `MamtaSheetDestination.decision`.
- Pushes `MamtaRoute.packageDetail`.
...
- `Analyze` can be available to Prateek/admin.
...
- Detail screens foreground Mamta adaptation.
...
- `Workspace.fixture`
- `Creator.fixtureMamta`
- `Member.fixtureMamta`
- `Member.fixturePrateek`
...
- App builds.
- Mamta/Admin mode switch works with fixture services.
...
- Daily Card Review shows Mamta Preview first.
...
- Pattern/Trend/Audio detail screens foreground Mamta adaptation and warnings.
...
- No Mamta screen exposes raw admin vocabulary beyond the approved small source note.
...
- Mamta Today shows one idea, no package preview, maximum two chips, and maximum two visible actions.
...
```text
Use build-ios-apps:swiftui-ui-patterns and build-ios-apps:ios-debugger-agent to scaffold a native iPhone-only SwiftUI app for Mamta Content OS V2.

Target iOS 26.0+.

Start with Slice 1 and Slice 2 only:
- App shell
- Mamta/Admin mode switch
...
Use docs/swiftui-build-implementation-spec-mamta-content-os-v2.md as the build contract.
Build and run on iOS Simulator, then visually verify the Mamta Today flow.
```
