# Mamta Content OS iPhone App V1 PRD

## Problem Statement

Mamta needs a simple daily creative routine for Instagram content. The current system lives across ChatGPT, a custom GPT, screenshots, trend links, notes, and manual coordination by Prateek. That works while Prateek is operating the system, but it is too scattered for Mamta to use independently each morning.

The app should turn a weekly setup pass by Prateek into one clear daily content card for Mamta. Mamta should open the app, see the best reel idea for today, decide whether she can shoot it, and get the shot list, script, caption, and audio notes without needing to prompt, research, or manage a content calendar.

## Product Direction

Working product: Mamta Content OS for iPhone.

V1 is bespoke for Mamta. The data model should support multiple workspaces and creators later, but the interface should expose only Mamta for now.

The app is a native SwiftUI iPhone-only app with a small Supabase backend for shared data, storage, sync, and AI calls. Mamta's phone should cache the current week locally so daily viewing and copying works offline. Generation, uploads, trend analysis, and sync require network.
...
- Mamta should not have to manage a content system.
...
Initial operator/admin. Maintains the creator profile, performs weekly setup, adds trend screenshots and links, reviews generated weekly plans, edits cards, and publishes the week to Mamta.
...
Prateek performs a 10-minute setup pass and generates a 7-day plan:
...
Published days are soft-locked. They change only when Mamta chooses an easier option, Prateek adds a strong trend, the schedule changes, a brand obligation appears, or an admin manually regenerates.
...
The app exports the Mamta Content OS custom GPT knowledge into an editable setup profile.
...
A completed day means Mamta made a decision, not necessarily posted.
...
If Mamta taps "Not today," the app immediately offers lower-effort options:
...
Use local scheduled notifications on Mamta's phone.
...
- Shared sync between Prateek's phone and Mamta's phone
...
- workspace_id
- daily_card_id
- revision_kind: ai_draft, admin_edit, mamta_edit, alternative, completion_update
- payload
- created_by_user_id
- created_at
...
- Pair Prateek device as owner/editor
- Pair Mamta device as creator
...
- Prateek creates and publishes a weekly plan on Sunday.
- Mamta receives a specific 8 AM notification daily.
- Mamta opens Today in under 3 seconds.
- Mamta can decide: shoot, easier, skip, save, or posted.
- Mamta can copy script, caption, and audio notes.
...
- Sync completion state from Mamta device to Prateek device
...
- Whether Prateek and Mamta share one TestFlight build configuration or use an internal debug flag for admin tools
- Exact notification time zone behavior when Mamta travels
