# Layers Conceptual Model: Mamta Content OS V2

Source reviewed: `/tmp/codex-remote-attachments/019e91d6-f797-7072-bc8c-b5db7482e6cf/D4864C26-EC4C-4E92-BC1D-B275F97C5DE7/1-mamta-content-os-v2-prd.md`
...
Product: Mamta Content OS, a native iPhone app that turns Prateek's weekly setup and curated source intelligence into one prepared, shootable daily content card for Mamta.

Core job:

- Prateek prepares the week.
- Mamta opens the app and knows exactly what to shoot today.
...
- Admin-only in V2; invisible to Mamta unless future multi-workspace support exists.
...
What it is: The editable memory that defines how Mamta should sound, what she posts about, and what the system should avoid.
...
What it is: Prateek's weekly input packet: the real-world context the next Weekly Plan should respect.
...
What it is: A curated set of benchmark creators or source rows Prateek wants the system to learn from.
...
What it is: A reusable content structure extracted from references or authored by Prateek.
...
What it is: A lightweight signal from Mamta or Prateek about a card, idea, pattern, trend, or output.
...
stateDiagram-v2
    ...
    Reviewed --> Published: admin publishes to Mamta
...
- Mamta should only see Published Weekly Plans by default.
...
stateDiagram-v2
    ...
    Published --> InDecision: Mamta opens or acts
...
stateDiagram-v2
    ...
    Candidate --> Approved: admin approves for Mamta
...
- Mamta's Today card should appear instantly from local cache.
- Admin changes can show "syncing" or "updated after refresh," but Mamta should not see blank daily content because sync is slow.

Engineering question:

- Whether to use optimistic updates for Mamta's completion states or queue them offline until confirmed.
...
Decision: Creator is the person content is for. In V2 this is Mamta.
...
Decision: Weekly Setup is the admin action/object Prateek prepares before generation.
...
Decision: Watchlist is the curated set Prateek manages. It can include the top 500 influencer list.
...
Decision: Feedback is a lightweight learning signal from Mamta or Prateek.
...
Decision: Prepare matches Prateek's weekly job.
...
Decision: Publish means Mamta can now see and use the plan. It does not mean Instagram posting.
...
Decision: Confirm means "the extraction is accurate." Approve means "this is fit for Mamta."
...
Decision: Surface language should be concrete for Mamta. Internally the state is Accepted.
...
- Backups can be stored on the Daily Card or as Ideas linked to it, but Mamta should experience them as lower-effort options for today.
...
- Approve fit for Mamta.
- Publish visibility to Mamta.
...
1. Should Mamta see source names for Trends and Patterns, or only the adapted idea and audio link?
...
7. Should Brand Brief approval status track brand approval, Prateek approval, or both separately?
...
9. Should Learning Summaries be visible to Mamta, or admin-only?
