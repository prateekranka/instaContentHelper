# Swift Data Boundary: Creator Content OS V2
...
- `CreatorContentOS/App/CreatorContentOSApp.swift` creates `AppRuntime`.
...
- `CreatorContentOS/Data/AppRepositories.swift`
...
The shared context object is `WorkspaceContext`, currently seeded with stable fixture IDs for Creator's workspace, creator, and member.

## Fixture Runtime

Fixture implementations live in:

- `CreatorContentOS/Fixtures/FixtureRepositories.swift`
...
- `CreatorContentOS/Data/SupabaseClientFactory.swift`
- `CreatorContentOS/Data/SupabaseDTOs.swift`
- `CreatorContentOS/Data/SupabaseRepositories.swift`
- `CreatorContentOS/Data/RuntimeConfigurationStore.swift`
- `CreatorContentOS/Data/DevicePairingService.swift`
...
The direct decision/archive write path is useful for dev/admin verification. Production device-paired writes should still move through Edge Functions before this is exposed to Creator's phone.

Device pairing path:

- Local config keys come from `CreatorContentOS/Config/Runtime.xcconfig`, optional `LocalRuntime.xcconfig`, app Info.plist keys, or process environment.
...
- Creator Today decisions write into Archive with optimistic local state and repository sync.
...
3. Manager publishes and soft-locks the week.
4. Creator Today reads the published card and works offline from cache.
5. Creator's decision writes to Archive.
6. Creator's phone schedules a local notification from the synced Today card.
