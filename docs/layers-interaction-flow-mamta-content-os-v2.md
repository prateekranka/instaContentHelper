# Layers Interaction Flow: Mamta Content OS V2

Source inputs:

- V2 PRD draft: `/tmp/codex-remote-attachments/019e91d6-f797-7072-bc8c-b5db7482e6cf/D4864C26-EC4C-4E92-BC1D-B275F97C5DE7/1-mamta-content-os-v2-prd.md`
- Conceptual model: `docs/layers-conceptual-model-mamta-content-os-v2.md`

This document defines interaction logic without committing to visual form.

## Flow Principles

- Mamta starts in Daily Mode. Prateek starts in Setup Mode.
...
- "Confirm" means extraction accuracy. "Approve" means fit for Mamta. "Publish" means Mamta can see it.
- Offline support is required for Mamta's Today flow, but not for generation, trend analysis, or publishing.
...
As Prateek, when a new week is approaching, I want to prepare Mamta's context, source material, brand obligations, and schedule, so that I can generate and publish a 7-day plan Mamta can use without prompting or researching.

### Success

A Reviewed Weekly Plan is Published to Mamta with seven Daily Cards, backups, warnings resolved or accepted, and notifications ready to schedule locally on Mamta's phone.
...
[ Shows current week status, next week start date, last published week, completion summary from previous week, unresolved warnings, and whether Mamta's current week is already published. ]
...
[ Shows exactly what Mamta will see, when the 8 AM notification will be scheduled, which cards have unverified audio, and which days include brand/event obligations. ]
...
[ Shows published state, sync status, Mamta completion states, and soft-lock notice. ]
...
- Mamta has completed some days: regenerated plan cannot overwrite completed days.
...
As Prateek or a scout, when I find a reel, audio, screenshot, or creator reference, I want to add it quickly and have the system extract useful meaning, so that only confirmed, Mamta-fit source material enters planning.
...
[ Shows analysis progress: identifying source, extracting audio, reading visible text, detecting hook/scene/caption/audio/brand patterns, scoring Mamta fit. ]
...
[ Shows raw Reference alongside extracted audio, creator handle, hook, visual format, caption formula, brand integration pattern, possible Pattern, possible Trend, possible Audio Option, possible Idea, fit score, and "what Mamta should avoid." ]
...
- Low Mamta Fit Score: default action should be dismiss or save for later, not use today.
...
As Mamta, when I open the app in the morning, I want one prepared idea with exactly what to shoot and post, so that I can decide quickly and keep momentum even if I cannot shoot the main reel.

### Success

Mamta makes a decision for today's Daily Card: shot, posted, used backup, saved for tomorrow, or skipped intentionally.
...
- If no Published Daily Card exists for today, show "No card published for today" with a message to ask Prateek.
...
- ask Prateek -> external message affordance if supported later

[ Shows offline explanation without blaming Mamta. ]
...
[ Shows lower-effort choices immediately. Does not ask Mamta to explain before offering alternatives. ]
...
- Mamta opens tomorrow's card accidentally: Today should default to current date, with a clear way back from Weekly Plan.
...
- Completion sync fails later: keep Mamta's local state, show admin sync warning to Prateek, not a scary error to Mamta.
...
As Prateek, when a brand obligation, key moment, strong trend, or schedule change appears midweek, I want to update only the affected future cards without destabilizing Mamta's completed days or surprising her.

### Success

The published Weekly Plan is updated deliberately: completed days are preserved, affected future cards are replaced/merged/backed up, and Mamta sees stable updated cards.
...
[ Shows changed cards, unchanged cards, notification implications, and whether Mamta's currently cached Today card will change. ]
...
- Mamta is currently viewing the card being updated: prefer non-disruptive sync. Show updated version next time she opens or after explicit refresh.
...
As Mamta or Prateek, when the current card is almost right but too hard, too long, not natural enough, or needs a new audio/trend angle, I want a safe preview alternative so I can choose it without losing the original.
...
- Mamta dismisses alternatives repeatedly: offer "Ask Prateek for help" or "Use backup story" rather than continuing endless generation.
- Admin accepts alternative on a future card already cached on Mamta's phone: sync as an update with revision history.
...
- Mamta's Today flow uses local cache first.
- Prateek's generation and analysis flows use pessimistic completion: no new plan/source object becomes usable until validation succeeds.
- Completion decisions by Mamta use optimistic local state and queue sync if offline.
...
- Mamta sees Daily Cards, package content, backup options, optional source explanation, completion state, and lightweight feedback.
- Prateek sees Weekly Setup, Weekly Plan, References, Watchlists, Patterns, Trends, Audio Options, Brand Briefs, Key Moments, Learning Summaries, and source provenance.
- Scout sees add-reference flow and pending status, not publish controls.

## Open Decisions

1. Should "Can shoot today" create a visible in-progress state for Mamta, or remain an unpersisted intent action?
2. Should Mamta see source names for Trends and Patterns on Today, or only the adapted idea and audio link?
...
6. Should completed Daily Cards be editable by Mamta after completion, or only by Prateek/admin?
7. Should local notification scheduling happen immediately on publish, or only when Mamta's device next syncs and confirms?
8. Should Watchlist import be part of the first TestFlight proof, or seeded manually to reduce first-build scope?
9. Should Brand Brief approval distinguish Prateek approval from brand approval?
10. Should failed generation expose raw AI validation errors to Prateek, or show simplified issue categories only?
...
- The Today flow assumes Mamta will accept a structured card/checklist experience. The first TestFlight week should be treated as observed-behaviour research.
