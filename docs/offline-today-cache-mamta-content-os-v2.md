# Offline Today Cache - Mamta Content OS V2

Slice 4 makes Mamta Today read the published card through the repository boundary and keep working when Supabase is unavailable.
...
- `todayCard`: the card Mamta should see first.
...
This preserves the multi-creator/workspace model while v1 still exposes only Mamta.
...
Covered in `MamtaContentOSTests/PublishWeekFixtureAcceptanceTests.swift`:

- Publishing a week stores the published Today card and seven week cards in cache.
- Mamta Today loads a cached published card.
