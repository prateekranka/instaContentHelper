# Supabase Schema: Mamta Content OS V2

This is the first backend slice for Mamta Content OS V2. It creates the Supabase-ready data foundation and the initial no-auth device pairing path.
...
- The V1 UI can stay bespoke to Mamta while the data model supports multiple creators/workspaces later.
...
1. Prateek creates a `device_invite`.
...
- Mamta Today: `daily_cards` by `creator_id + scheduled_date` where the card is published or completed.
- Prateek Weekly Control: `weekly_plans` by `creator_id + week_start_date + status`, then `daily_cards` by `weekly_plan_id`.
