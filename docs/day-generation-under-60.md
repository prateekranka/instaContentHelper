...
**Production path (2026-07-28+): async thumbs.** Day generation marks the run
**script-ready** (poll returns draft/complete) as soon as text validates and
persists. Gemini storyboard thumbnails attach in the background with the same
prompt/quality path; soft-fail does not fail the day. Primary SLA gate is
**time-to-script-ready** p95 < 60s (~19/20). Time-to-full-visuals is a secondary
metric.

This benchmark still supports bounded storyboard concurrency as the preferred
architecture for the visual lane itself. With the same deterministic text,
validation, persistence, and five-visual workload, virtual p50/p95 total time
improved from 48.616/49.726 seconds for serial storyboard work to
37.755/38.699 seconds with two visual lanes. Three lanes measured 32.557/33.569
seconds in the same offline model. Production defaults to **five** visual lanes
(`DEFAULT_STORYBOARD_THUMBNAIL_CONCURRENCY = 5`) so five Gemini thumbs finish in
one wave. The offline gate still requires concurrency-2 to pass so the
architecture remains proven at the conservative setting.

Live stage timings (2026-07-28, sync thumbs on critical path) showed text at
~37–60s and visuals at ~16–31s; combined success p95 remained ~84s, which is why
visuals moved off the critical path. DeepSeek V4 maps `low`/`medium` → `high`,
and a non-thinking first pass usually failed validation then recovered via a
thinking repair — paying both costs. Day flash therefore keeps thinking enabled
at `high` so the first pass can succeed in one call. Day status polling uses
`poll_after_seconds: 2` to cut client wait overhead.

Day generation attaches Gemini storyboard thumbnails **server-side after**
script-ready completion (`visuals_status: pending` → `ready`). Clients may see
the script before thumbs; status polls hydrate assets from `daily_cards`, and
the iOS UI can still gap-fill via `generate-storyboard-thumbnail`.
