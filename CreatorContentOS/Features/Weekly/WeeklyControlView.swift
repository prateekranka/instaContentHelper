    ...
    var body: some View {
        JournalBlock {
            VStack(alignment: .leading, spacing: MCOSpace.m) {
                ...
                GeneratedStoryboardBreakdownContent(card: card) { assets in
                    card.storyboardThumbnailAssets = assets
                }
                GeneratedTextEditor(title: "Edit scene titles", text: sceneListBinding, minHeight: 96)
...
struct WeeklyDayDetailSheet: View {
    ...
    private var plannedContent: some View {
        if let generatedCard {
            GeneratedDayPlannedContent(card: generatedCard) { assets in
                self.generatedCard?.storyboardThumbnailAssets = assets
            }
        } else {
